...
docker-compose up -d
```
*Note: This binds the database to port `5433` on your host machine to avoid conflicts with any existing local Postgres instances.*
...
node setup-db.mjs
```
*(You should see a success message confirming `init.sql` was executed).*
